...
for i = 1:length(Participants)
    for j = 1:length(Participants(i).Sessions)
        disp([Participants(i).Sessions(j).nbackPartition ' is procesing ...'])
        ...
        FreqBand([5,6,10,11,12]) = [];
        FreqBand(end+1).Range = [0.5 30];FreqBand(end).Name = 'All';

        df=Fs/size(Segments,3);

        for f = 1:length(FreqBand)
            Dataset_Train1 = Segments;
            FreqRange=FreqBand(f).Range;   
            Dataset_Freq = [];
            for seg = 1:size(Dataset_Train1,1)
                clear tmpseg tmpseg2
                tmpseg(:,:) = Segments(seg,:,:)-mean(Segments(seg,:,:),3);
                Wtmp = tmpseg .* repmat(tukeywin(size(tmpseg,2))',size(tmpseg,1),1);
                tmp1 = abs(fft(Wtmp,size(Wtmp,2),2));        
                tmpseg2 = tmp1(:,round((FreqRange(1)/df)+1):round((FreqRange(2)/df)));
                Dataset_Freq(seg,:,:) = tmpseg2;
            end
            FreqBand(f).fftDataset = Dataset_Freq;

            % Dist - Freq - Euclidean
            DistMetricType = 'euclidean'; % Dsitance types : 'euclidean', 'cosine', 'correlation'
            DistFreqs = [];
            [DistFreqs] = DistGenerator(FreqBand(f).fftDataset, DistMetricType);
            FreqBand(f).EuclidDistDatasets = DistFreqs;

            % Dist - Freq - Cosine
            DistMetricType = 'cosine'; % Dsitance types : 'euclidean', 'cosine', 'correlation'
            DistFreqs = [];
            [DistFreqs] = DistGenerator(FreqBand(f).fftDataset, DistMetricType);
            FreqBand(f).CosDistDatasets = DistFreqs;

            % Dist - Freq - Correlation
            DistMetricType = 'correlation'; % Dsitance types : 'euclidean', 'cosine', 'correlation'
            DistFreqs = [];
            [DistFreqs] = DistGenerator(FreqBand(f).fftDataset, DistMetricType);
            FreqBand(f).CorrDistDatasets = DistFreqs;

        end

        try
            save(Participants(i).Sessions(j).nbackPartition,'FreqBand');
        catch
            warning('Saved as version v7.3......');
            save(['V73',Participants(i).Sessions(j).nbackPartition],'FreqBand','-v7.3');
        end
    end
end
...
for i = 1:length(PrePostParticipants)
    ...
    for f = 1:length(PrePostParticipants(i).Sessions(1).FreqBand)
        ...
        tmpPre = PrePostParticipants(i).Sessions(1).FreqBand(f).EuclidDistDatasets;
        tmpPost = PrePostParticipants(i).Sessions(2).FreqBand(f).EuclidDistDatasets;
        ...
        for j = 1:length(PrePostParticipants)
            ...
            tmpPre = PrePostParticipants(j).Sessions(1).FreqBand(f).EuclidDistDatasets;
            tmpPost = PrePostParticipants(j).Sessions(2).FreqBand(f).EuclidDistDatasets;
            NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        try
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Euclid_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test');
        catch
            warning('Saved as version v7.3......');
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Euclid_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'V73.mat'],...
        ...
        tmpPre = PrePostParticipants(i).Sessions(1).FreqBand(f).CosDistDatasets;
        tmpPost = PrePostParticipants(i).Sessions(2).FreqBand(f).CosDistDatasets;
        NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        for j = 1:length(PrePostParticipants)
            ...
            tmpPre = PrePostParticipants(j).Sessions(1).FreqBand(f).CosDistDatasets;
            tmpPost = PrePostParticipants(j).Sessions(2).FreqBand(f).CosDistDatasets;
            NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        try
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Cos_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test');
        catch
            warning('Saved as version v7.3......');
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Cos_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'V73.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test','-v7.3');
        end
        ...
        tmpPre = PrePostParticipants(i).Sessions(1).FreqBand(f).CorrDistDatasets;
        tmpPost = PrePostParticipants(i).Sessions(2).FreqBand(f).CorrDistDatasets;
        NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        for j = 1:length(PrePostParticipants)
            ...
            tmpPre = PrePostParticipants(j).Sessions(1).FreqBand(f).CorrDistDatasets;
            tmpPost = PrePostParticipants(j).Sessions(2).FreqBand(f).CorrDistDatasets;
            NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        try
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Corr_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test');
        catch
            warning('Saved as version v7.3......');
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Corr_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'V73.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test','-v7.3');
        end
        ...
        tmpPre = PrePostParticipants(i).Sessions(1).FreqBand(f).Po2Corr3DistDatasets;
        tmpPost = PrePostParticipants(i).Sessions(2).FreqBand(f).Po2Corr3DistDatasets;
        NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        for j = 1:length(PrePostParticipants)
            ...
            tmpPre = PrePostParticipants(j).Sessions(1).FreqBand(f).Po2Corr3DistDatasets;
            tmpPost = PrePostParticipants(j).Sessions(2).FreqBand(f).Po2Corr3DistDatasets;
            NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        try
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Po2Corr3_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test');
        catch
            warning('Saved as version v7.3......');
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Po2Corr3_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'V73.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test','-v7.3');
        end
        ...
        tmpPre = PrePostParticipants(i).Sessions(1).FreqBand(f).Po2Corr2DistDatasets;
        tmpPost = PrePostParticipants(i).Sessions(2).FreqBand(f).Po2Corr2DistDatasets;
        NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        for j = 1:length(PrePostParticipants)
            ...
            tmpPre = PrePostParticipants(j).Sessions(1).FreqBand(f).Po2Corr2DistDatasets;
            tmpPost = PrePostParticipants(j).Sessions(2).FreqBand(f).Po2Corr2DistDatasets;
            NumSeg = min(size(tmpPre,1),size(tmpPost,1));
        ...
        try
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Po2Corr2_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test');
        catch
            warning('Saved as version v7.3......');
            save(['J:\Concussion\NewDataset\Datasets\Datasets_CleanSegments_PrePost_Freq_Po2Corr2_',PrePostParticipants(i).Sessions(1).FreqBand(f).Name,'_ID_',num2str(PrePostParticipants(i).ID),'V73.mat'],...
                  'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test','-v7.3');
        end   
    end
    toc
...
%     
%     
%     load(Participants(i).Sessions(1).nbackPartition)
%     
%     
%     
%     %Labels are 0 or 1
...
%    
%    
%    ['Datasets_CleanSegments_PrePost_Freq_Euclid_' Alpha-Beta '_ID_' 1 '.mat'];
%    
%     
%     
% 
...
%         
%         [Dataset_Train,Labels_Train,Dataset_Test,Labels_Test] = ...
%             PatientSpecDatasetsGenerator (FreqBand(f).PatientsDatasets, PatNum);
% 
%         try
%             save(['Datasets_CleanSegments_PrePost_Freq_' FreqBand(f).Name '_Pt_' num2str(PatNum) '.mat'],...
%                   'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test');
%         catch
%             warning('Saved as version v7.3......');
%             save(['V73_Datasets_CleanSegments_PrePost_Freq_' FreqBand(f).Name '_Pt_' num2str(PatNum) '.mat'],...
%                   'Dataset_Train','Dataset_Test','Labels_Train','Labels_Test','-v7.3');
%         end
%         clear Dataset_Train Labels_Train Dataset_Test Labels_Test

%% Clean datasets - Partitioning

% DatasetList = dir('..\Dataset\TBIC edf- baselinecorrected_Clean');
% 
% Fs = 250;
% 
% for DSNum = 3:length(DatasetList) 
% 
%     tic;
%     tmpfile = [DatasetList(DSNum).folder '\' DatasetList(DSNum).name];
%     disp([DatasetList(DSNum).name ' is procesing ...'])
%     try 
%         [tmpheader, tmprecorddata] = edfread(tmpfile);
%         m = tmprecorddata(end,:);
%         n = numel(m);
%         m2 = interp1(1:n, m, linspace(1, n, (tmpheader.samples(1)/tmpheader.samples(end))*n), 'nearest');
%         tmprecorddata(end,:) = m2(1:size(tmprecorddata,2));
...
% 
%         try
%             save(['TBIC edf- baselinecorrected_Clean_Partitioned\ ',DatasetList(DSNum).name,'_FreqPartitions.mat'],'FreqBand');
% 
%         catch
%             warning('Saved as version v7.3......');
%             save(['TBIC edf- baselinecorrected_Clean_Partitioned\ ',DatasetList(DSNum).name,'_FreqPartitionsV73.mat'],'FreqBand','-v7.3');
%         end
%     catch
%         disp([DatasetList(DSNum).name ' ...Had issues..!!!!!!!'])
%     end
%     toc
% end

%% Segmented - filter datasets - Partitioning

% DatasetList = dir('..\Dataset\TBIC edf files- filtered & segmented');
% 
% Fs = 250;
% 
% for DSNum = 3:length(DatasetList) 
...
% 
%         try
%             save(['TBIC edf files- filtered & segmented_Partitioned\ ',DatasetList(DSNum).name,'_FreqPartitions.mat'],'FreqBand');
% 
%         catch
%             warning('Saved as version v7.3......');
%             save(['TBIC edf files- filtered & segmented_Partitioned\ ',DatasetList(DSNum).name,'_FreqPartitionsV73.mat'],'FreqBand','-v7.3');
%         end
%     catch
%         disp([DatasetList(DSNum).name ' ...Had issues..!!!!!!!'])
%     end
